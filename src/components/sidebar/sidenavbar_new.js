...
import { profileMenu, getMenuItems, settingsTabs, multiWorkSpaceProfileMenu } from "./sidenavbaroptions_new";
...
class sidenavbar2 extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      currentOpenKey: [],
      jiraSkill: false,
      checkinSkillDisabled: false,
      projectDisabled: false,
      helpdeskDisabled: false,
      reportsDisabled: false,
      wikiSkillDisabled: false,
      sidebarVisible:true
  ...
  switchView = (e) => {
    ...
    let sub_skill = e.keyPath[1]
    //console.log("key:", e);
    // if (sub_skill && pathArr[2] === 'skills'){
    //   const tab = key.split(":")[1];
    //   let link = `/${wId}/skills/${pathArr[3]}/${sub_skill}`
    //   if(tab) link += `?view=${tab}`
    ...
    if (key === "teamsyncs") {
      ...
      localStorage.setItem("TROOPR_DRIFT_APP_MESSAGE", "👋")
      this.openChatWindow("👋");
    } else if (key === "logout") {
      this.logoutFromApp();
    ...
    // } else if (key.startsWith("Jira")) {
      ...
      // } else if (key === "grid_metrics") {
      //   this.props.history.push(`/${wId}/grid_metrics`);
    } else if (key === "metrics") {
      this.props.history.push(`/${wId}/metrics`);
    } else if (key && key.length > 0 && key.startsWith("Jira") || key.startsWith("jira")) {
        if (sub_skill){
          const id = this.state.jiraSkill._id
          const tab = key.split(":")[1];
          let link = `/${wId}/skills/${id}/${sub_skill}`
          if(tab) link += `?view=${tab}`
          this.props.history.push(link)
        }else {
          const tab = key.split(":")[1];
    ...
    } else if (key && key.length > 0 && key.startsWith("wiki")) {
      const tab = key.split(":")[1];
      this.goToSkill(key.split("wiki")[1], tab);
      // this.props.history.push(`/${wId}/skills/${key.split("GitHub")[1]}`);
    } else if (key === "upgrade") {
    ...
    } else if (key === "global-insights") {
      this.props.history.push(`/${wId}/global-insights`);
    } else if (key === "planned_absences") {
      this.props.history.push(`/${wId}/${key}`);
    }
    else if (key === "globalholidays") {
      this.props.history.push(`/${wId}/globalholidays`)
    }
    else if (key === "mynotifications") {
      this.props.history.push(`/${wId}/settings?view=notifications`);
    } else if (key === "labels") {
      this.props.history.push(`/${wId}/labels`);
    } else if (settingsTabs.includes(key)) {
      this.props.history.push(`/${wId}/settings?view=${key}`);
    } else if (key === "appHome") {
      this.props.history.push(`/${wId}/appHome`);
    } else if (key === "getting_started") {
      this.props.history.push(`/${wId}/getting_started`);
    ...
    if (e.keyPath && e.keyPath.length === 1) {
      //this is not a submenu item, so close any open submenu
  ...
  componentDidMount() {
    ...
    //console.info("this.currentpage",this.state.currentpage)
    if(skills.length > 0 ){
      this.checkSkillDisabled();
    }
    ...
    if (wId) {
      ...
      if (
        currentpage == "teamsyncs" ||
        currentpage == "global-insights" ||
        currentpage == "myabsences" ||
        currentpage == "teamsync_integrations" ||
        (currentpage && currentpage.length > 0 && currentpage.startsWith("ts"))
      ) {
        this.setState({ currentOpenKey: ["standups_inline"] });
      } else if (
        (currentpage && currentpage.length > 0 && currentpage.startsWith("sqd")) ||
        currentpage == "squads" ||
        currentpage == "labels" ||
        currentpage == "mynotifications" ||
        currentpage == "notifications"
      ) {
        this.setState({ currentOpenKey: ["squads"] });
      } else if (currentpage == "jira_user_welcome") {
        this.setState({ currentOpenKey: "jira_inline" });
      } else if (currentpage == "jiraConnectionSteps") {
        this.setState({ currentOpenKey: "jira_inline" });
      } else if (currentpage == "dashboard") {
        this.setState({ currentOpenKey: [] });
      }
      ...
      this.props.getWorkspaceMembers(wId).then(res => {
        if (res.data.success) this.checkIfWorkspaceAdmin();
      });

      this.props.getAssisantSkills(wId).then((res) => {
        if (res.data.success) {
          const { currentpage } = this.state;
          ...
          let { resource, pathname } = this.props.location;
          let pathArr = pathname.split("/");

          if (pathArr[2] && pathArr[2] === "skills") {

            let currentSkill = this.props.skills.find((s) => {
              let id = s.skill_metadata ? s.skill_metadata._id : s._id;
              return id === pathArr[3];
            })

            if (currentSkill) {
              const sub_skill = pathArr[4]
              if (sub_skill && pathArr[2] === 'skills') {
                this.setState({ currentOpenKey: sub_skill })
              }
              else if (currentSkill.name == "Jira") {

                this.setState({ currentOpenKey: ["jira_inline"] });
              } else if (currentSkill.name == "GitHub") {
                this.setState({ currentOpenKey: ["github_inline"] });
              } else if (currentSkill.key == "wiki") {
                this.setState({ currentOpenKey: ["wiki_inline"] });
              }
            }
      ...
      if (workspace && workspace.showSquads) {
        this.props.getProjects(wId);
        if (currentpage && currentpage.length > 0 && currentpage.startsWith("sqd")) {
          let pId = currentpage.split('sqd')[1]
          this.props.recentProjects(pId, wId)
        } else {
          this.props.getRecentProjects(wId);
        }
      }

      if (currentpage && currentpage.length > 0 && currentpage.startsWith("ts")) {
        let tId = currentpage.split('ts')[1]
        this.props.recentCheckins(tId, wId)
      } else {
        this.props.getRecentTeamsyncs(wId);
      }
  ...
  componentDidUpdate(prevProps, prevState) {
    const { userTeamSync, workspace, skills } = this.props;
    ...
    if (wID && this.state.wId !== wID) {
      ...
      this.props.getWorkspaceMembers(wID).then(res => {
        if (res.data.success) this.checkIfWorkspaceAdmin(wID);
      });
    ...
    if (this.props.location.pathname !== prevProps.location.pathname) {
      ...
      if (this.props.location.pathname.split("/")[4]) {
        ...
        if (currentpage && currentpage.startsWith('ts') || currentpage.startsWith(`teamsyncs`))
          this.setState({/* currentpage, */currentOpenKey: ['standups_inline'] })
        else /* handling sub skill */
          this.setState({/* currentpage, */currentOpenKey: [this.props.location.pathname.split("/")[4]] })
      }
      else if (currentpage.startsWith("ts")) this.setState({ currentpage });
      else if (currentpage.startsWith("sqd")) this.setState({ currentpage });
      else if (currentpage === "teamsyncs") this.setState({ currentpage, currentOpenKey: ["standups_inline"] });
      else if (currentpage === "squads") this.setState({ currentpage });
      else if (currentpage === "dashboard") this.setState({ currentpage, currentOpenKey: [] });
      else if (currentpage.startsWith("Jira")) this.setState({ currentpage, currentOpenKey: ["jira_inline"] });
      else if (currentpage.startsWith("wiki")) this.setState({ currentpage, currentOpenKey: ["wiki_inline"] });
      // to handle check-in onboarding
      else if (currentpage === "templates") this.setState({ currentpage });
      else if (currentpage === "jira_user_welcome") this.setState({ currentpage, currentOpenKey: ["jira_inline"] });
      else if (currentpage === "jiraConnectionSteps") this.setState({ currentpage, currentOpenKey: ["jira_inline"] });
    ...
    if ((!prevProps.currentSkill && this.props.currentSkill) || (prevProps.currentSkill && this.props.currentSkill && prevProps.currentSkill._id !== this.props.currentSkill._id)) {
      let { pathname } = this.props.location;

      // console.log("cView",this.state.currentOpenKey)
      const data = pathname.split("/")[2];
      if (data === "skills") {
        const jiraSkill = this.props.skills.find((skill) => skill.name === "Jira");
        // const githubSkill = this.props.skills.find((skill) => skill.name === "GitHub");
        let cView = this.getCurrentView()
        // console.log("cView",cView)
        if (cView.startsWith("Jira")) {
          if (jiraSkill && !jiraSkill.skill_metadata.disabled) this.setState({ currentpage: cView });
        } else this.setState({ currentpage: cView });
      }
    }
  }
  ...
  checkSkillDisabled = () => {
    const { skills } = this.props
      ...
      this.setState({
        jiraSkill : jiraSkill.skill_metadata,
        checkinSkillDisabled: checkinSkill.skill_metadata.disabled,
        projectDisabled: jiraProject.disabled,
        reportsDisabled: jiraReport.disabled,
        helpdeskDisabled: jiraHelpdesk.disabled,
        wikiSkillDisabled: wikiSkill.skill_metadata.disabled,
      })
  }
  ...
  checkIfWorkspaceAdmin = (wId) => {
    ...
    if (members && members.length > 0) {
      const currentUserFound = members.find(user => user.user_id._id === user_now._id)
      if (currentUserFound) this.setState({ isAdmin: currentUserFound.role === 'admin' ? true : false })
      else this.setState({ isAdmin: false });
    }
  };
  ...
  goToSkill = (data, tab) => {
    const skill_id = data ? data.split(":")[0] : [];
  ...
  toggleSubMenu = (e, sub_skill, skill) => {
    // console.info("e.key",e)
    const { currentpage } = this.state;
    if (e && e.key) {
      this.setState({
        currentOpenKey: [e.key],
      });
      if (sub_skill && skill) {
        this.props.history.push(`/${this.props.match.params.wId}/skills/${skill.skill_metadata._id}/${sub_skill.key}?view=connection`);
        // console.log(`/${this.props.match.params.wId}/skills/${skill.skill_metadata._id}/${sub_skill.key}?view=connection`)
        this.setState({ currentpage: sub_skill.key + skill.skill_metadata._id + ":connection" });
      }
      else if (e.key == "standups_inline") {
        //this.setState({currentpage:"teamsyncs"})
        this.props.history.push(`/${this.props.match.params.wId}/teamsyncs`);
      } else if (e.key == "jira_inline" || e.key == "jira_inline") {
        if (this.props.skills) {
          let jiraSkill = this.props.skills.filter((skill) => skill && skill.name === "Jira");
          if (jiraSkill && jiraSkill[0].skill_metadata) {
            this.props.history.push(`/${this.props.match.params.wId}/skills/${jiraSkill[0].skill_metadata._id}?view=connection`);
            this.setState({ currentpage: "Jira" + jiraSkill[0].skill_metadata._id + ":connection" });
          }
        }
      } else if (e.key == "github_inline") {
        if (this.props.skills) {
          let githubSkill = this.props.skills.filter((skill) => skill && skill.name === "GitHub");
          if (githubSkill && githubSkill[0].skill_metadata) {
            this.props.history.push(`/${this.props.match.params.wId}/skills/${githubSkill[0].skill_metadata._id}?view=connection`);
            this.setState({ currentpage: "GitHub" + githubSkill[0].skill_metadata._id + ":connection" });
          }
        }
      } else if (e.key == "wiki_inline") {
        if (this.props.skills) {
          let wikiSkill = this.props.skills.filter((skill) => skill && skill.key === "wiki");
          if (wikiSkill && wikiSkill[0].skill_metadata) {
            this.props.history.push(`/${this.props.match.params.wId}/skills/${wikiSkill[0].skill_metadata._id}?view=connection`);
            this.setState({ currentpage: "wiki" + wikiSkill[0].skill_metadata._id + ":connection" });
          }
        }
      } else if (e.key == "squads") {
  ...
  getCurrentView = () => {
    ...
    if (resource === "squad") {
      ...
      if (this.props.skills) {
        ...
        // console.log(   cView )
        if (sub_skill) {
          cView += sub_skill + pathArr[3]
          cView += query.view ? `:${query.view}` : ":connection";
        } else if (skill && skill.key === 'wiki') {
          cView = skill ? skill.key + pathArr[3] : "";
          cView += query.view ? `:${query.view}` : ":connection";
        }
        else if (cView.startsWith("Jira") || cView.startsWith("GitHub")) {
      ...
      cView = pathArr[3]
        ? pathArr[3] === "all" || pathArr[3] === "mycheckins" || pathArr[3] === "templates"
          ? "teamsyncs"
          : pathArr[3] === "integrations"
            ? "teamsync_integrations"
            : "teamsyncs"
        : resource;
    } else if (resource === "settings") {
      const query = queryString.parse(window.location.search);
      // cView = query.view || "profile";
      cView = query.view || "settings";
    } else {
      cView = resource;
    }
    return cView;
  ...
  getMultiWorkSpaceMenu = (data, getId) => {
    const { assistant_skills } = this.props;
    const workspace = data;
    const id = getId;
    const switcherFunc = this.onWSSwitcherMenu;
    return (
      <Menu
        onClick={({ item, key, keyPath, domEvent }) => {
          domEvent.preventDefault();
        ...
        onClick={this.switchView}
      >
        {multiWorkSpaceProfileMenu({ assistant_skills }, workspace, id, switcherFunc)}
      </Menu>
    );
  }
  ...
  getMenuItems = () => {
    ...
    const { userTeamSync, workspace, skills, recentPro, projects, project, teamSyncs, user_now, currentSkill, members, team, recentTeamsyncs } = this.props;
    ...
    const toggleSubMenu = (e, sub_skill, skill) => this.toggleSubMenu(e, sub_skill, skill);
  ...
  isOnlyCheckinEnabled = () => {
    const { checkinSkillDisabled, projectDisabled, reportsDisabled, helpdeskDisabled, wikiSkillDisabled } = this.state
    if(!checkinSkillDisabled && projectDisabled && reportsDisabled && helpdeskDisabled && wikiSkillDisabled) return true
    else return false
  };

  isOnlyProjectEnabled = () => {
    const { checkinSkillDisabled, projectDisabled, reportsDisabled, helpdeskDisabled, wikiSkillDisabled } = this.state
    if(checkinSkillDisabled && !projectDisabled && reportsDisabled && helpdeskDisabled && wikiSkillDisabled) {
      return true
    } else { return false }
  };

  isOnlyHelpdeskEnabled = () => {
    const { checkinSkillDisabled, projectDisabled, reportsDisabled, helpdeskDisabled, wikiSkillDisabled } = this.state
    if(checkinSkillDisabled && projectDisabled && reportsDisabled && !helpdeskDisabled && wikiSkillDisabled) return true
    else return false
  };

  isOnlyReportsEnabled = () => {
    const { checkinSkillDisabled, projectDisabled, reportsDisabled, helpdeskDisabled, wikiSkillDisabled } = this.state
    if(checkinSkillDisabled && projectDisabled && !reportsDisabled && helpdeskDisabled && wikiSkillDisabled) return true
    else return false
  };
  ...
  isOnlySquadsEnabled = () => {
    ...
    if (
      ...
      jiraSkill &&
      jiraSkill.skill_metadata.disabled &&
      this.checkFeaturesVisible("squads") &&
      !this.checkFeaturesVisible("github")
    )
      return true;
    else return false;
  };
  helperFunction = () => {
    let userName;
    if (this.props.user_now && (this.props.user_now.name || this.props.user_now.displayName)) {
      userName = this.props.user_now.name || this.props.user_now.displayName;
    } else userName = "";
    let workspaceName = this.props.workspace.name;
    workspaceName = workspaceName.length > 24 ? `${workspaceName.slice(0, 21)}...` : workspaceName;
    userName = userName.length > 24 ? `${userName.slice(0, 21)}...` : userName;
    return (
      <a className="upper-menubar">
        <Avatar
        ...
        >
          {this._getInitials(
            this.props.user_now.displayName || this.props.user_now.name
          )}
        </Avatar>
        <div
          style={{
  ...
  multiWorkSpaceUpperMenuBar = (workspace, wId) => {
    return (
      <Tooltip title={this.props.workspace.name} placement='right'>
        <Dropdown
          overlay={this.getMultiWorkSpaceMenu(workspace, wId)}
          trigger={["click"]}
        >
          {this.helperFunction()}
        </Dropdown>
  ...
  updateBot = () => {
    window.open('https://app.troopr.io/slack?source=botupdate', "_self")
  }

  updateBotBanner = () => {
    return (
      <Alert
        style={{ width: "100vw", minHeight: "40px", maxHeight: "40px", zIndex: "10000", position: "absolute" }}
        message={
          <span>
            It appears that you may have an older version of Troopr installed. It is recommended that you update to the latest version to take full advantage of Troopr's capabilities.{" "}
            <Button onClick={() => this.updateBot()} type='primary' size="small">
              Update Troopr
            </Button>{" "}
          </span>
        }
  ...
  render() {
    ...
    // }
    // console.info("currentPage",this.state.currentpage)
    // console.log("current key=====> " + this.state.currentOpenKey)
    const { workspace, location, members,user_now, team } = this.props;
    const { currentpage } = this.state;
    // console.log("cView",this.state.currentOpenKey, currentpage)
    let wId = this.props.match.params.wId;
    let userId = localStorage.getItem("trooprUserId");
    ...
    if (user_data) this.props.user_now.displayName = user_data.user_id.displayName || ''

    const botScopesArr = team && team.bot && team.bot.scope ? team.bot.scope.split(',') : []
    let oldBot = false
    if (team && team.bot && team.bot.scope && botScopesArr.length > 0 && !botScopesArr.includes('groups:write')) oldBot = true

    return (
      <>
      {oldBot && this.updateBotBanner()}
      <Fragment>
        {/* {workspace &&
          ...
          <Sider
            ...
            onCollapse={(collapsed, type) => {
              ...
              } else this.setState({ sidebarVisible: false })
            }}
          >
            <Menu
              className=' submenu_class dashboard'
              theme={"dark"}
              ...
              //defaultOpenKeys={this.isOnlyCheckinEnabled() ? ["standups_inline"] : this.isOnlyJiraEnabled() ? ['jira_inline'] : this.isOnlySquadsEnabled()?['squads']:[]}
              openKeys={this.isOnlyCheckinEnabled() ? ['standups_inline'] : this.isOnlyProjectEnabled() ? ['jira_software'] : this.isOnlyHelpdeskEnabled() ? ['jira_service_desk'] : this.isOnlyReportsEnabled() ? ['jira_reports'] : this.isOnlyWikiEnabled() ? ["wiki_inline"] : this.state.currentOpenKey}
            ...
            >
              {
                ...
                */
                <>
                  {isMultiWorkspace ? (
                    this.multiWorkSpaceUpperMenuBar(workspaces, wId)
                  ) : (
                    this.upperMenuBar()
                  )}
                </>
              }
              {this.state.sidebarVisible ? this.getMenuItems() : ""}
            </Menu>
          </Sider>
