...
class Jiraoauth extends Component {
  ...
  SaveConnection = async () => {
    ...
    let data = await getOauthTokens(this.props.match.params.wId, this.state.sessionid)
  ...
  next = async () => {
    ...
    if (parseInt(this.props.match.params.current) == 2) {
      this.setState({ loading: true })
      let info = await getRSAKeyPair(this.props.match.params.wId, this.state.domainurl,/* sub_skill */ this.state.parsedQueryString.sub_skill)
      let data = { publicKey: info.publicKey, consumerkey: info.consumerkey, sessionid: info.sessionid }
      this.props.addKeysAndSession(data).then(() => this.props.history.push(`/${this.props.match.params.wId}/jiraoauthServer/${this.props.match.params.skill_id}/3${this.state.parsedQueryString.sub_skill  ? '?sub_skill=' + this.state.parsedQueryString.sub_skill : ''}`))
      ...
      if (parseInt(this.props.match.params.current) == 0) {
        if (!this.state.domainurl) {
          ...
          const goToNextPage = async () => {
            ...
            if (validUrlInfo) {
              ...
              this.props.addDomainURL(this.state.domainurl, instanceInfo).then(() => {
                this.props.history.push(`/${this.props.match.params.wId}/jiraoauthServer/${this.props.match.params.skill_id}/1${this.state.parsedQueryString.sub_skill  ? '?sub_skill=' + this.state.parsedQueryString.sub_skill : ''}`)
              })
            }
          }
          ...
          if( validURL(domainurl) && domainurl.includes('atlassian.net')){
            Modal.confirm({
              title: 'It appears that you are connecting to a Jira Cloud site, click "Proceed" to continue to connect to Jira Cloud. Click "Cancel" to continue to connect to Jira Server/DC site.',
              okText: 'Proceed',
              // maskClosable : true,
              keyboard : false,
              onOk: () => this.goToCloudOauth(),
        ...
        this.props.history.push(`/${this.props.match.params.wId}/jiraoauthServer/${this.props.match.params.skill_id}/2${this.state.parsedQueryString.sub_skill  ? '?sub_skill=' + this.state.parsedQueryString.sub_skill : ''}`)
      }
    }
  }
  ...
  previous = () => {
    ...
    this.props.history.push(`/${this.props.match.params.wId}/jiraoauthServer/${this.props.match.params.skill_id}/${current - 1}${this.state.parsedQueryString.sub_skill  ? '?sub_skill=' + this.state.parsedQueryString.sub_skill : ''}`)
    ...
    const steps = [
      {
        title: "Jira URL",
        content: (
          <DomainUrl domainURL={this.state.domainurl} isVerified={this.state.isVerified} loading={this.state.loading} showError={this.state.showError} error={this.state.error} next={this.next} addUrl={this.addUrl} isCloud={false} verify={this.verify} />
        )
      },
      {
        title: "Create Link",
